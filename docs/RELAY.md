...
- **현재 담당:** Claude → Codex 인계 대기 (PM이 대체 구현 계획과 변경점을 Codex에 직접 전달 예정)
...
0. **[Codex 착수 대상] 마우스 조준 코어 구현** — `docs/superpowers/plans/2026-07-28-mouse-aim-combat-core.md`(보완 문서 §8 인용, 기존 core 플랜 Task 1~5 대체)의 Task 1(상태·aim 이행)부터 Task 5(인계)까지 순서대로. R-3(틱 파이프라인 순서 normative 고정)·P-1(카운터 만료 스모크)이 계획에 포함돼 있다. Task 1 상태 코드(`fd7044d`·`2f39e48`)는 계획 Task 1이 이행하고, 구 Task 2(`7148034`)는 계획 Task 2가 교체한다. 완료 기준: 각 Task의 스모크·빌드 관측이 ai-log에 기록되고 커밋된다.
