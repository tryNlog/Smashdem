...
/**
 * 런당 3택1 리롤 허용 횟수(§17-D / systems-designer §18-5 권고 N=2).
 * "3택 전부 빗나간 판" 구제용 — 무제한이면 3택1 이 1택이 되어 선택압이 죽는다(§17-D 무제한 반려).
 * 재플레이에서 짜증(PT-2-2)이 남으면 3 으로 승격 여지(U16, 완주율 급변 시 PM 상신).
 * ※ 이 값은 런 구조 상수다. 물리·데미지·세트 배율 같은 밸런스 상수와 별개다.
 */
export const RUN_REROLL_COUNT = 2;
